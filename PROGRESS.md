...
pytest.ini                 [ĐÃ TẠO] `pythonpath = .` — bắt buộc phải có để `pytest` (không chỉ `python -m pytest`) tìm thấy package `app/`; không có file này, chạy trực tiếp `python tests/test_x.py` hoặc bare `pytest` sẽ báo `ModuleNotFoundError: No module named 'app'`
...
- **Đã tự phát hiện lúc viết `tests/` (Giai đoạn 8)**: bare `pytest` (khác `python -m pytest`) không tự thêm thư mục gốc repo vào đường dẫn tìm module nếu `tests/` không có `__init__.py` — sẽ báo `ModuleNotFoundError: No module named 'app'`. Đã sửa bằng file `pytest.ini` (`pythonpath = .`) ở gốc repo — **bắt buộc phải có file này** thì lệnh `pytest` trong `ci.yml` mới chạy đúng trên runner GitHub.
