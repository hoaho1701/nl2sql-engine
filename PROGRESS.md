...
- **⚠️ Giai đoạn 0 KHÔNG cài được trên máy đang code hiện tại — đã tự kiểm chứng, không phải chưa thử đúng cách.** Máy đang dùng (kiểm tra 2026-09-14): Intel MacBook Pro cũ, macOS 12.7.5 (Monterey), i7-4770HQ, 16GB RAM, hostname `Hoas-MacBook-Pro-2.local` — KHÔNG phải Mac Mini M4. Đã tự verify 2 điểm chặn cứng:
  ...
  - **Rủi ro lớn nhất của cả dự án là thời gian chờ máy M4, không phải kỹ thuật** — 8/11 giai đoạn phụ thuộc hoàn toàn vào việc chuyển máy. **Đã quyết định (2026-09-22): dù bị trễ bao lâu, KHÔNG dùng phương án tốn phí để né việc chờ** (GitHub Codespaces có GPU, thuê GPU cloud theo giờ...) — đây sẽ vi phạm thẳng ràng buộc $0 xuyên suốt dự án (xem "Mục tiêu sản phẩm"), không đánh đổi lấy tốc độ. Việc duy nhất làm được khi bị trễ: tiếp tục phần infra-độc-lập còn lại rồi đợi, không tìm đường vòng.
...
- **README chính thức viết SAU CÙNG, không viết trước** — cần số liệu thật (từ `results/eval_log.csv`) và demo thật (từ Giai đoạn 6-9 đã chạy được), viết sớm sẽ phải sửa lại nhiều lần và có nguy cơ ghi sai kỳ vọng so với thực tế. **Ngoại lệ đã áp dụng (2026-09-22)**: 1 bản `README.md` **placeholder** ngắn (gắn nhãn 🚧 Work in Progress) đã viết sớm hơn kế hoạch — vì quyết định đưa repo lên Public sớm (coi "xây công khai" là tín hiệu tích cực, không phải rủi ro), không thể để repo public trống trơn không có README nào. Placeholder này **không phải** bản chính thức — vẫn phải thay hẳn bằng bản đầy đủ (đúng thứ tự bên dưới) khi có số liệu thật.
- **Chuẩn bị hồ sơ (resume bullet, talk-track phỏng vấn) làm SONG SONG lúc code, khác với README** — README cần chờ vì cần số liệu thật để không viết sai, nhưng resume bullet/talk-track dùng để *kể lại quyết định kỹ thuật đã có*, không cần chờ dự án xong 100%. Chuẩn bị dần tránh dồn hết vào cuối.
...
- **Draft resume bullet dần, điền số liệu thật khi có** (không đợi xong mới viết) — vd:
...
- **Chuẩn bị talk-track phỏng vấn (60-90 giây)** — tập trung đúng 2-3 quyết định kỹ thuật khó nhất, không kể lại toàn bộ 11 giai đoạn:
  1. Vì sao chọn Postgres/RAG thay vì nhồi schema tĩnh — trade-off cụ thể (xem "Quyết định tech stack" ở Giai đoạn 0).
  2. Câu chuyện phát hiện lỗ hổng CTE-ghi-dữ-liệu ở Giai đoạn 5 — chi tiết hiếm, đáng kể nhất trong toàn bộ dự án.
  3. Cách đo execution accuracy bằng `Counter` thay vì so văn bản SQL, và vì sao train/eval phải tách biệt (Giai đoạn 7).
- **Sau khi có README chính thức**: cập nhật GitHub profile README (`hoaho1701/hoaho1701`, pin repo này) và bất kỳ trang portfolio tổng hợp nào khác trỏ đúng vào link README mới — dễ quên bước đồng bộ ngược lại sau khi 1 project xong.
